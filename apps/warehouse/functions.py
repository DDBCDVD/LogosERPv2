...
def create_unit_package(request, pckg):
    units_create = False
    for qty in range(int(pckg.unit_qty)):
    ...
    if unit_ids:
        for unit_id in unit_ids:
            stock_control.objects.create(
                quantity = int(pckg.product_id.measure_qty),
                unit_id = unit_id,
                location_id = pckg.location_id
            )
            unit_id.quantity = int(pckg.product_id.measure_qty)
...
def create_stock_move(request, move):
    ...
    if move.package_id:
        ...
        product_package.location_from_id_id = location_id
        product_package.location_id_id = location_dest_id
        product_package.unit_qty = move.quantity
        ...
        if  unit_ids:
            for unit_id in unit_ids:
                ...
                unit_id.location_from_id = location_id
                unit_id.save()
        move.save()
    ...
    if not origin_data:
        ...
        if move.location_dest_id.location_type  == 'Internal':
            if not dest_data:
                ...
                if move.package_id:
                    origin_data.quantity = origin_data.quantity - origin_data.quantity
                elif move.unit_id:
                    #~ Calcula el saldo de la unidad en el almacén
                    move.prev_qty = unit_id.quantity
            ...
            if origin_data.quantity == 0.0:
                ...
                if unit_id.package_id:
                    package_id = products_package.objects.get(id=unit_id.package_id.id)
